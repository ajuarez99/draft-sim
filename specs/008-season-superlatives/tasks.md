...
- [X] T001 Confirm the branch and migration state before writing anything:
...
- [X] T002 Start Postgres on 5433 (`docker compose up -d`; start Docker Desktop first if `docker ps`
...
- [X] T003 Take the **before** measurements from quickstart §1 and write them to a new
...
- [X] T004 Create `backend/src/main/resources/db/migration/V22__season_superlatives.sql` with four
...
- [X] T005 Add three abstract methods (no `default` body) to `…/sport/SportRules.java`, each
...
- [X] T006 [P] Implement them in `…/sport/FootballRules.java`:
...
- [X] T007 [P] Implement them in `…/sport/BasketballRules.java`:
...
- [X] T008 [P] Add `test/…/sport/SuperlativeSportRulesTest.java` with fixtures copied from research
...
- [X] T009 Move `visibleLeague(String sleeperId, String sleeperUserId)` and
...
- [X] T010 Create `…/store/WeekBound.java`: `public record WeekBound(Integer throughWeek)` with
...
- [X] T011 Thread `WeekBound` through the score and pairing queries:
...
- [X] T012 Thread `WeekBound` through `…/engine/LeagueRecordService.java`:
...
- [X] T013 Create `…/engine/SeasonSuperlativesService.java` with the payload records from
...
- [X] T014 Create `…/api/SuperlativesController.java` with
...
- [X] T015 [P] Mirror T013's records in `web/src/api.ts`:
...
- [X] T017 Create `web/src/pages/Superlatives.tsx`:
...
- [X] T018 Create `web/src/pages/Superlatives.test.tsx` checking that an `available: false` payload
  shows its reason, and that a kind with `available: false` renders its reason, not an empty card.

- [X] T019 Decide roster membership for the Embiid and Unethical awards from T003's R11 result
...
- [X] T020 [P] [US1] Add ordering tests in `test/…/engine/SeasonSuperlativesCloseGamesTest.java`,
...
- [X] T021 [US1] Implement `CLOSE_WINS` and `CLOSE_LOSSES` in
...
- [X] T022 [US1] Implement `HIGHEST_WEEK`, `LOWEST_WEEK`, `BIGGEST_BLOWOUT` and `CLOSEST_GAME` in
...
- [X] T023 [US1] Add coverage for weeks with scores but no pairings (US1 scenario 7): weeks in the
  ...
  file.
- [X] T024 [US1] Render the six US1 kinds in `web/src/pages/Superlatives.tsx`:
...
- [X] T025 [US1] Extend `web/src/pages/Superlatives.test.tsx`: a two-holder tie renders both names;
  the close-wins card prints the payload's margin; a coverage note renders "N of M weeks".
- [X] T026 [US1] Live-check US1 per quickstart §2 (SC-001 and SC-007 by hand against the stored
...
- [X] T027 [P] [US2] Add bound tests in `test/…/engine/ExpectedWinsServiceTest.java`: with games in
...
- [X] T028 [US2] Change `…/engine/ExpectedWinsService.java` so `forLeague(String sleeperLeagueId,
...
- [X] T029 [US2] Update `…/api/ExpectedWinsController.java` to pass the league's regular-season
  bound via the T028 helper. The Expected wins page now shows regular-season luck.
- [X] T030 [US2] Update `…/engine/ManagerCareerService.java:225`
...
- [X] T031 [P] [US2] Add ordering tests in `test/…/engine/SeasonSuperlativesLuckTest.java`:
...
- [X] T032 [US2] Implement `LUCKIEST` and `UNLUCKIEST` in `…/engine/SeasonSuperlativesService.java`
  ...
  - Unit `WINS`, and a one-line reading built from `winsAboveExpected`, e.g. "2.40 more wins than
    their scores earned".
- [X] T033 [US2] Implement `MOST_BENCH_POINTS` in `…/engine/SeasonSuperlativesService.java`:
...
- [X] T034 [US2] Set `early = true` on `LUCKIEST`, `UNLUCKIEST` and `MOST_BENCH_POINTS` (and, later,
  the US3, US4 and US6 kinds) when `weeksScored < EARLY_THRESHOLD_WEEKS`, in
  `…/engine/SeasonSuperlativesService.java`.
- [X] T035 [US2] Render the US2 kinds in `web/src/pages/Superlatives.tsx`: the figure with its
  ...
  shows the caveat.
- [X] T036 [US2] Live-check SC-002 on both NFL leagues. Then compare the Expected wins page's NFL
...
- [X] T037 [P] [US3] Add rule tests in `test/…/engine/WaiverPickupAttributionTest.java`, one per case:
...
- [X] T038 [US3] Implement `…/engine/WaiverPickupAttribution.java` as a pure static function over
...
- [X] T039 [US3] Wire `WAIVER_WIRE_WARRIOR` into `…/engine/SeasonSuperlativesService.java`:
  ...
  - Coverage lists weeks with null `starters`, and never falls back to all rostered points (US3
    scenario 4).
  - `early` per T034.
- [X] T040 [US3] Render the pickup card in `web/src/pages/Superlatives.tsx` (total, top pickups
  with week added and how acquired), and add a test to `web/src/pages/Superlatives.test.tsx`.
- [X] T041 [US3] Live-check SC-003 by hand for the NFL 2026 holder, and settle research R8's `leg`
...
- [X] T042 [P] [US4] Create `…/store/PlayerAbsenceRepository.java`: `record Row(Sport sport, int
...
- [X] T043 [P] [US4] Add ingest tests in `test/…/ingest/PlayerGameIngestServiceTest.java` using
...
- [X] T044 [US4] Change `…/ingest/PlayerGameIngestService.java`:
...
- [X] T045 [US4] Run `POST /api/ingest/player-games/1254190892974084096` and
  ...
  (quickstart §1 R9).
- [X] T046 [P] [US4] Add ordering and rule tests in `test/…/engine/AbsenceCostTest.java`:
...
- [X] T047 [US4] Implement `…/engine/AbsenceCost.java` as a pure function per research R10
  (amended):
...
- [X] T048 [US4] Wire `JOEL_EMBIID` into `…/engine/SeasonSuperlativesService.java`:
  ...
  - `early` per T034.
- [X] T049 [US4] Render the Embiid card in `web/src/pages/Superlatives.tsx`:
...
- [X] T050 [US4] Live-check quickstart §3 on the NBA 2025 league. Hand-count the holder's top
...
- [X] T051 [P] [US6] Create `…/store/StatusCaptureRepository.java`:
...
- [X] T052 [US6] Change `…/ingest/PlayerIngestService.java`: after `players.upsertAll`, call
...
- [X] T053 [P] [US6] Add a test in `test/…/ingest/PlayerIngestServiceSuspensionTest.java`: given a
...
- [X] T054 [P] [US6] Create `…/store/LeagueConductRepository.java`:
...
- [X] T055 [US6] Add the conduct-list endpoints to `…/api/SuperlativesController.java` per
...
- [X] T056 [P] [US6] Add `test/…/api/SuperlativesControllerIT.java` (Postgres-backed, same setup
...
- [X] T057 [P] [US6] Add ordering and rule tests in `test/…/engine/UnethicalAwardTest.java`:
...
- [X] T058 [US6] Wire `UNETHICAL` into `…/engine/SeasonSuperlativesService.java`:
  ...
  - Set `suspensionWeeksObserved` (regular-season captured weeks) and `commissionerListAvailable`
    (`anyCommissioner`).
  - `early` per T034.
- [X] T059 [US6] Mirror the conduct-list payloads in `web/src/api.ts` (`ConductList`,
...
- [X] T060 [US6] Render the Unethical Award and the list in `web/src/pages/Superlatives.tsx`:
...
- [X] T061 [US6] Live-check quickstart §4 **from the browser page** (POST/DELETE preflight,
...
- [X] T062 [P] Add `test/…/engine/NoSportNameInSuperlativesTest.java`, modelled on
  ...
  `Sport.NFL`/`Sport.NBA`.
- [X] T063 [P] Grep the new and changed Java files for `Map.of(` on a response path and replace any
  that can hold a null (`reason`, `coverage`, `emptyReason`, `gamesMissed`, `regularSeasonEnd`,
  `addedBy`).
- [X] T064 Run the full backend suite (`cd backend && ./gradlew test`) with Postgres up, and **read
...
- [X] T065 Run `cd web && npx tsc -b && npm run build && npx vitest run` and record the result.
- [X] T066 Run quickstart §5 in the browser on `1346366555759341568` and `1229352720222134272`:
...
- [X] T067 Run the pipeline's bug-hunting code review over the branch diff (`/code-review`, not a
...
- [X] T068 Update `HANDOFF.md`: what shipped, what was verified vs assumed, the Expected wins
...
- [X] T070 Update the memory `project_season_superlatives.md` from "planned, not built" to its real
