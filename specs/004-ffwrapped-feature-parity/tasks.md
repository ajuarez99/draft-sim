...
- [X] T127 [US6] Ingested transactions for all five remaining leagues on 2026-09-19 via `POST /api/ingest/transactions/{id}`. Stored: `1141438340626231296` 635, `1229352720222134272` 1618, `1254190892974084096` 471, `1262506916429430784` 378, `1389361939561332736` 29 — 3,131 rows against the 16 this phase opened with. All six leagues now hold transactions. Note a type the contract did not anticipate: `COMMISSIONER` (26 rows across three leagues), alongside `FREE_AGENT`, `WAIVER` and `TRADE`
- [X] T128 [US6] US6.5 verified on real NBA data, not a fixture: `GET /api/leagues/1229352720222134272/transactions` returns 12 managers, 1,231 adds and 16 trades, with `postMovePositionalRank` resolved through **basketball** positions (`"position": "PG"`, ranks 16.9 / 44.0 / 14.1 with `weeksCounted` beside each). The sport-agnostic claim now rests on real rows
- [X] T129 [US6] US6.3 verified against real trades, which did not exist when this phase was written (`TRADE` count was 0 league-wide). There are now 25 trade rows across three leagues; the NBA league renders 16 with both sides, the players each side received, each player's post-move positional rank, and `rankDirection: "LOWER_IS_BETTER"` stated explicitly. The 18-row / 16-trade gap is correct behaviour, checked rather than assumed: two rows are pick-only trades with empty `adds`/`drops`, which have no players to grade and are rightly excluded

### Weekly Report starters backfill (US5)

- [X] T130 [US5] Backfilled `starters` on 2026-09-19 by re-running `POST /api/ingest/league-history/{id}` for `1262506916429430784` (216 weeks re-ingested) and `1389361939561332736` (2 seasons, 26 weeks). Verified by counting populated rows rather than by the response: `roster_week_points` is now **986 rows, 986 with starters — zero nulls in all six leagues**, against 756/986 when this phase opened. The extended skip gate from T082 did refetch the null-starter weeks, so plan.md Risk 2 held up under test
- [X] T131 [US5] Moot, and recorded as such: Sleeper **did** return `starters` for every settled week, so there is no unavailability to document under R6 and no league whose Weekly Report awards are permanently degraded. The `awardsOmitted` / `STARTERS_NOT_STORED` path remains correct and tested, but no ingested league now exercises it
...
- [X] T133 [US4] Deleted `NO_DISTRIBUTIONS` from all three declarations — `PlayoffOddsService.java` (the enum), `web/src/api.ts` (the union) and `web/src/pages/SeasonForecast.tsx` (the refusal branch). **Deleted rather than emitted**, deliberately: a pre-V17 snapshot still carries real `playoffOdds` and `averageWins` and lacks only `winRange`, `averageSeed` and `seedOdds`, so it is served `available: true` and degrades field by field. Promoting it to a whole-page refusal would have hidden numbers the snapshot genuinely has. The enum's javadoc now states that absence as a decision, and two stacked javadoc blocks on that declaration were merged into one while there. Suites after: backend `489 tests, 0 failures, 0 skipped`; frontend `441 passed`; `tsc --noEmit` clean

### Phase 10 verification

- [X] T134 Re-ran both suites and re-measured every count this phase is keyed on, 2026-09-19. Backend **489 tests, 0 failures, 0 skipped**, read from `backend/build/test-results/test/*.xml` rather than the build result; frontend **42 files, 441 tests passing**; `tsc --noEmit` clean. The four counts, then → now: populated `starters` 756/986 → **986/986**; `league_transaction` 16 rows in one league → **3,147 across all six**; `TRADE` 0 → **25** (16 renderable, 2 pick-only, correctly excluded); NBA forecast `NOT_COMPUTED` → **`available: true` for both NBA leagues**, each with 12 entries carrying seed and win distributions
