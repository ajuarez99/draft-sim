...
- [X] T081 Run the full backend suite and **read the skip count**, not the build result: `grep -ho 'skipped="[0-9]*"' backend/build/test-results/test/*.xml | sort | uniq -c` — `BUILD SUCCESSFUL` is printed with integration tests silently skipped when Postgres is down — run 2026-09-21 17:49 local: **83 result files, 83 test classes in `backend/src/test`, `skipped="0"` and `failures="0"` on every one**, so no integration test was silently skipped. No backend source has changed since that run (verified with `find backend/src -newer`); only `ManagerHistory.tsx` and its test are newer, which T082 covers — re-run 2026-09-22 with Postgres up: 83 classes, 565 tests, `skipped="0"` and `failures="0"`. The same run with Docker down reports BUILD SUCCESSFUL with **86 tests silently skipped**, `ManagerHistorySportIT` among them
- [X] T082 Run `web` tests with `npm test` in `web/` — re-run 2026-09-22 after the final `ManagerHistory.tsx` edits: 43 files, 474 tests, all passing
- [X] T083 Walk every scenario in [quickstart.md](quickstart.md) end to end and record the results — done 2026-09-22 against live Postgres and both servers run from the 006 worktree, signed in as popsharky. Every scenario passes; the four places where the live answers differ from what quickstart.md predicted are written up in its own "Results" section, and none is a defect. The champion gate was re-proved rather than observed clearing: Sleeper still answers `in_season` with a winner id, and a fresh ingest of all three chains leaves exactly four placements
- [X] T084 Update `README.md`'s status section with what shipped
