...
- [ ] T083 Walk every scenario in [quickstart.md](quickstart.md) end to end and record the results — **blocked 2026-09-22**: Docker Desktop is not running on this machine, so there is no Postgres on 5433 and no backend to walk it against. Scenario 2 is the one that still needs live proof: re-running the ingest must *clear* the two champions already stored on in-progress 2026 seasons, not merely stop writing new ones
